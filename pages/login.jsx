...
const OAUTH_ERROR_MESSAGES = {
  discord_not_configured: "Discord login isn't configured on this server yet.",
  discord_cancelled: "Discord login was cancelled.",
  discord_missing_params: "Discord didn't return the expected info — please try again.",
  discord_state_mismatch: "Login session expired. Please try Discord login again.",
  discord_token_exchange: "Couldn't connect to Discord. Please try again.",
  discord_already_linked: "That Discord account is already linked to a different user.",
  discord_email_clash: "Another account is already using the email on your Discord profile.",
  registration_closed: "Registration is currently closed.",
  banned: "Your account is banned.",
};

export default function Login({ server, discordEnabled }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const dest = router.query.next || "/account";
      router.push(dest);
    } catch (e) {
    ...
    <Layout user={null} server={server}>
      <div className="container page" style={{ maxWidth: 420 }}>
        <div className="card card-pad" style={{ padding: 32 }}>
          ...
          {displayedErr && <div className="alert alert-error">{displayedErr}</div>}

          {/* Discord OAuth — kicks off the OAuth flow */}
          {discordEnabled ? (
            <>
              <a
                href={`/api/auth/discord${router.query.next ? `?next=${encodeURIComponent(router.query.next)}` : ""}`}
